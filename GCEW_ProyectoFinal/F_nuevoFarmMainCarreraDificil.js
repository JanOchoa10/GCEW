...
cityScene.background = new THREE.Color("#b29126");
...
const terrainTexture = terrainTextureLoader.load("../images/farmFloor.jpg");
...
function updatePlayerMovement() {
  ...
  if (isThePlayerCollisionRobot) {
    movementSpeed = getRobotLessSpeed();
  } else {
  ...
  if (isThePlayerCollisionRobot) {
    ...
    powerUpTimer = setTimeout(() => {
      isThePlayerCollisionRobot = false;
...
function getRobotLessSpeed() {
  return isShiftPressed ? robotMovementSpeed : robotMovementSpeed;
}
...
var modelBBBadRobot2;
let fbxBadRobot2;

function loadAnimatedModelAndPlayBadRobot2() {
  const loader = new FBXLoader();
  loader.setPath("../resources/robot/");
  loader.load("robotReal.fbx", (loadedfbx) => {
    fbxBadRobot2 = loadedfbx;
    fbxBadRobot2.scale.setScalar(0.1);
    fbxBadRobot2.rotateY(Math.PI / 2);
    fbxBadRobot2.traverse((c) => {
      c.castShadow = true;
    });
    fbxBadRobot2.position.copy(new THREE.Vector3(5, 0, 0.5143906859829599));

    // Crear la caja de colisión para el modelo animado
    modelBBBadRobot2 = new THREE.Box3().setFromObject(fbxBadRobot2);

    const animLoader = new FBXLoader();
    animLoader.setPath("../resources/robot/");
    animLoader.load("robotReal.fbx", (anim) => {
      const mixer = new THREE.AnimationMixer(fbxBadRobot2);
      animationMixer.push(mixer);
      const idleAction = mixer.clipAction(anim.animations[0]);
      idleAction.play();
...
loadAnimatedModelAndPlayBadRobot2();
...
var initialPositionIA2 = new THREE.Vector3(5, 0, 0.5143906859829599);
var finalPositionIA2 = new THREE.Vector3(70, 0, 0.5143906859829599);
var initialBBPositionIA2 = initialPositionIA2.clone();
var finalBBPositionIA2 = finalPositionIA2.clone();


var directionIA2 = finalPositionIA2.clone().sub(initialPositionIA2).normalize();
var distanceIA2 = finalPositionIA2.distanceTo(initialPositionIA2);

var animationDurationIA2 = 1.5; // Duración de la animación en segundos
var animationTimeIA2 = 0;
var animationSpeedIA2 = distanceIA2 / animationDurationIA2;

function animateMovementRobot2(delta) {
  if (animationTimeIA2 < animationDurationIA2) {
    // Calcular la nueva posición basada en el tiempo de animación
    var newPositionIA2 = initialPositionIA2.clone().add(directionIA2.clone().multiplyScalar(animationTimeIA2 * animationSpeedIA2));

    // Calcular la diferencia entre la posición inicial y final del modelo
    var positionOffset2 = newPositionIA2.clone().sub(initialPositionIA2);

    // Actualizar la posición del modelo
    fbxBadRobot2.position.copy(newPositionIA2);

    // Actualizar la posición de la caja de colisión
    modelBBBadRobot2.min.add(positionOffset2);
    modelBBBadRobot2.max.add(positionOffset2);

    // Actualizar la posición inicial y final de la caja de colisión
    initialBBPositionIA2.add(positionOffset2);
    finalBBPositionIA2.add(positionOffset2);

    animationTimeIA2 += delta;
  } else {
    // Invertir la dirección para el siguiente ciclo de movimiento
    directionIA2.multiplyScalar(-1);

    // Intercambiar las posiciones inicial y final para el siguiente ciclo de movimiento
    var tempPosition2 = initialPositionIA2.clone();
    initialPositionIA2 = finalPositionIA2.clone();
    finalPositionIA2 = tempPosition2.clone();

    // Intercambiar las posiciones inicial y final de la caja de colisión para el siguiente ciclo de movimiento
    var tempBBPosition2 = initialBBPositionIA2.clone();
    initialBBPositionIA2 = finalBBPositionIA2.clone();
    finalBBPositionIA2 = tempBBPosition2.clone();

    animationTimeIA2 = 0; // Reiniciar el tiempo de animación para el siguiente ciclo

    // Rotar el modelo 90 grados a la derecha
    fbxBadRobot2.rotateY(Math.PI);
  }

  checPowerRobotCollision2();
}
...
function checPowerRobotCollision2() {
  if (modelBBBadRobot2.intersectsBox(jugadorBB)) {
    console.log("Colisión con el modelo del powerup");
    isThePlayerCollisionRobot = true;
...
function animate() {
  ...
  animateMovementRobot2(deltaTime);
  //emitSnowParticles();
  //actualizarJugador();
  renderer.render(cityScene, camera);
}
